...
EOF
  ...
  if [ "${ENVIRONMENT}" = "prod" ] && [ -n "${AUTHENTIK_TS_CLIENT_ID:-}" ]; then
    ...
    chmod 700 "${TS_ROSTER_DIR}"
    {
      ...
      echo "# Live TS analyst roster. Edit via 'vote grant' / 'vote revoke'."
      echo "# Format: email=role   (roles: admin, analyst, reader)"
  ...
  if [ -f /etc/vote-case.env ]; then
    ...
    if [ -n "${CASE_ID}" ]; then
      ...
      if [ "${ENVIRONMENT}" = "prod" ] && [ -n "${AUTHENTIK_OR_CLIENT_ID:-}" ]; then
        ...
        {
          echo "# Generated by install.sh on $(date -u +%FT%TZ)"
          echo "# Live OR analyst roster. Edit via 'vote grant' / 'vote revoke'."
          echo "# Format: email=role   (roles: admin, analyst, reader)"
...
PYEOF
  ...
  #   email            → role defaults to reader
  #   email:role       → role ∈ {admin, analyst, reader} (CYPFER taxonomy)
  # CYPFER → VR mapping: admin→administrator, analyst→analyst, reader→reader.
  # Runtime changes go through 'vote grant' / 'vote revoke' → vr-apply.sh.
...
EOF
  ...
  {
    echo "# Generated by install.sh on $(date -u +%FT%TZ)"
    echo "# Live VR analyst roster. Edit via 'vote grant' / 'vote revoke'."
    echo "# Format: email=role   (roles: admin, analyst, reader)"
    if [ -n "${VR_BOOTSTRAP_USERS:-}" ]; then
